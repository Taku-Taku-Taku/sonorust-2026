...
pub async fn speaker(
    ...
    let (model_name, speaker_names, is_model_26_more) = {
        let client = handler.infer_client.read().await;
        match client.as_ref() {
            Either::Left(python_client) => {
                get_python_speakers(python_client.model_info(), userdata)
            }
    ...
    let embed = {
        ...
        CreateEmbed::new()
            .title(format_t!("speaker.embed.title", lang, model_name))
            .description(content)
    };
...
fn create_button_row() -> CreateActionRow {
    let page_back = CreateButton::new(lang_t!("customid.page.speaker.back"))
        .label("<-")
        .style(ButtonStyle::Primary)
        .disabled(true);

    let page_number = CreateButton::new(lang_t!("customid.page.speaker.number"))
        .label("1")
        .style(ButtonStyle::Secondary)
        .disabled(true);

    let page_forward = CreateButton::new(lang_t!("customid.page.speaker.forward"))
        .label("->")
        .style(ButtonStyle::Primary)
        .disabled(false);
...
pub fn create_command(lang: Lang) -> CreateCommand {
    CreateCommand::new("speaker").description(lang_t!("speaker.command.description", lang))
}
